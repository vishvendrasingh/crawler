#!/usr/bin/python
#########################################USAGE###############################################
#$Surface Crawler By Vishvendra Singh                                                       #
#$crawler_3.0.py 8 http://example.com example.com                                           #
#$crawler_3.0.py workers Full URL mongo_collection_name                                     #
#Details - Overload is sent to thrread 1, rest of the load is distributed as equal to others#
#############################################################################################
'''START IMPORT HERE'''
import threading
import requests
...
for i in range(1,no_of_thread+1):
    ...
    t = threading.Thread(target=worker,args=(i,to_send))
    t.start()
